...
## Part 2 - Setting up an empty Implementation Guide
### 2.1 - Verify Prerequisites
Before proceeding, ensure that all required tools are installed correctly on your system. Run the following commands in your terminal to verify that you have the correct versions installed:
* **Java**: `java --version`
* **Node.js**: `node -v`
* **Jekyll**: `jekyll -v`
* **Ruby**: `ruby -v`

If any of the versions are incorrect or you don't get a version, refer back to the prerequisites section and follow the links for installation instructions. 

### 2.2 Initialize the IG
To initialize a new FHIR Implementation Guide, you can use the `sushi init`command. This command scaffolds a basic IG structure, including configuration files and folders that are necessary for your project. Follow these steps:

1. Open your terminal and navigate to the folder where you want to create an IG. 
2. Run the following command to initialize the IG:
    ```bash
    sushi init
    ``` 
    This will prompt you to provide details like the IG's name, canonical URL, and publisher. Feel free to provide information, or leave it blank to use the default values. 

Alternatively, if you want to use a custom DHIS2 FHIR IG template, you can clone this repo, which includes the necessary setup and exercise steps: 
```bash
git clone https://github.com/JohanGHole/dhis2-tb-tracker-ig-tutorial.git
```
This will provide a ready-made structure and configuration for the DHIS2 tracker to FHIR exercise.

### 2.3 - Navigate to the FSH folder
Once the IG is initialized, navigate to the `input/fsh` folder. This is where you will define all your FHIR artifacts (code systems, value sets and logical models). For the purposes of this exercie, we will set up a folder structure that follows the [SMART Guidelines IG authoring conventions](https://smart.who.int/ig-starter-kit/authoring_conventions.html). 

If you have initialized your own IG, create the following folders within `input/fsh`to organize your FSH files:
* `input/fsh/codesystems` for defining custom code systems.
* `input/fsh/valuesets`for defining value sets.
* `input/fsh/models`for logical models.

You should now have a basic, empty IG ready to be populated with FSH. The next steps will involve adding custom artifacts based on DHIS2 metadata to these folders using FSH, which we will handle in the next sections. 
## Part 3 - Handling Option Sets: Creating Code System and Value Sets Using FSH
### Goal
Define FHIR code systems and value sets based on DHIS2 option sets. 

### 3.1 - Create FHIR Code Systems:
...
* Example:
    ...
    * #"Female" "Female"
    ```
### 3.2 - Map Code Systems to Value Sets
...
## Part 4 - Define the Logical Models for the Program Stages
...
## Part 5 - Define the Logical Model for the Tracker Program
...
### 5.1 - Start by defining the logical model for the TB Tracker Program
...
### 5.2 - Link Program Stages to the Logical Model
...
## Part 7 (Bonus) - Add Narrative Content
...
### 7.3 - Update Pages in `sushi-config.yaml`
...
### 7.4 - Update the Menu in `sushi-config.yaml`
